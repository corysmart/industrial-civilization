...
def main() -> int:
    ...
    expected_categories = {
        "possession": 18, "operation": 27, "mastery": 18,
        "construction": 10, "transition": 2, "research": 2,
    }
    if len(milestones) != 145: failures.append(f"expected 145 quests, found {len(milestones)}")
    if retrieval != 77: failures.append(f"expected 77 retrieval tasks, found {retrieval}")
    if advancement != 68: failures.append(f"expected 68 advancement tasks, found {advancement}")
    if dict(categories) != expected_categories:
        failures.append(f"retrieval categories differ: {dict(categories)}")
    if proxy_covered != 59:
        failures.append(f"expected independent evidence for 59 proxies, found {proxy_covered}")

    if failures:
        for failure in failures: print(f"FAIL: {failure}")
        return 1
    print("QUEST ACCEPTANCE: PASS")
    print("quests=145 advancement=68 retrieval=77 possession=18 proxies=59")
    print("proxy categories: operation=27 mastery=18 construction=10 transition=2 research=2")
    print("independent runtime/live evidence routes=59/59")
    return 0
