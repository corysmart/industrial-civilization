...
- IC2 Classic packet handling and MFSU output were verified from the installed 1.5.11 bytecode, and the work model has deterministic 1/4/10/50-source tests. Live tests now prove one- and four-source aggregation, 1x/4x GUI telemetry, native illegal-packet destruction, the scientific time floor, real Fabricator/Assembler/Robotic/Gun recipes, and save/reload continuation. Physical 10/50-MFSU cabling, an orbital burst factory, live Matter Replicator scaling, and ComputerCraft attachment remain stress-test coverage rather than unverified core semantics.
...
- The Technic client currently launches with legacy Java 8u74 and LWJGL2. On macOS it can intermittently fail before Forge initialization in `MacOSXContextImplementation.makeCurrent`/`objc_release`; a retry normally passes, but this native launcher-runtime failure cannot be corrected by pack scripts or mod code.
